...
public class BSTMap<K, V> implements Map<K, V> {
	...
	public V put(K key, V value) {
		...
		//...
		Entry<K,V> nuevo = new MapEntry<K, V>(key, value);
		if (get(key)!=null) { 
			Position<Entry<K, V>> target = tree.getPosition(new MapEntry<>(key, null));
			target.setElement(nuevo);
			return value;
		}
		else {
			tree.addElement(nuevo);
			return value;    // for the moment...
		}
	}
	...
	public V remove(K key) {
		...
		// them to instance field tree.... EXERCISE 1
		//...
		Entry<K,V> nuevo = new MapEntry<K, V>(key, null);
		if (get(key)==null) 
			return null;
		else 
			nuevo = tree.removeElement(nuevo);
		return nuevo.getValue();    // for the moment...
	}
	...
	public Iterable<K> keySet() {
		...
		//...
		ArrayList<K> target = new ArrayList<>();
		Iterable<Position<Entry<K,V>>> iter = tree.positions();
		for (Position<Entry<K, V>> position : iter) {
			target.add(position.getElement().getKey());
		}
		return target;    // for the moment...
	}
	...
	public Iterable<V> values() {
		...
		// inner class......  EXERCISE 2
		//...
		ArrayList<V> target = new ArrayList<>();
		Iterable<Position<Entry<K,V>>> iter = tree.positions();
		for (Position<Entry<K, V>> position : iter) {
			target.add(position.getElement().getValue());
		}
		return target;    // for the moment...
	}
